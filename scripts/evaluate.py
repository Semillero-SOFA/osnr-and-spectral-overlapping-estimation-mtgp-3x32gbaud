...
def evaluate(model_path=None, batch_size=1024):
    # Auto-detect model path if not provided
    if model_path is None:
        model_path = Path(f"checkpoints/processed/mixed_gp_model.pth")
        if not model_path.exists():
            raise FileNotFoundError(f"Model not found at {model_path}")

    # Load Data (Test set only ideally, but load_dataset returns both)
    print(f"Loading processed data for evaluation...")
    _, _, test_x, test_y, scaler_x, scaler_y = load_dataset()
    ...
    dummy_inducing = torch.zeros(num_inducing, num_features)
    ...
    with torch.no_grad():
        for batch_x, batch_y in tqdm.tqdm(test_loader):
            ...
            # Overlap (Task 0) - use likelihood to get probabilities
    ...
    targets_osnr = scaler_y.inverse_transform(
        targets_osnr_norm.reshape(-1, 1)
    ).flatten()
    ...
    print(f"Overlap ROC-AUC: {roc_auc:.4f}")
    ...
    # Save results
    results_dir = Path(f"results/processed")
